...
export async function fetchSceneLocationsByTmdb(
  ...
  if (opts?.movieInfo?.originalTitle)
    params.set("originalTitle", opts.movieInfo.originalTitle);
  if (opts?.movieInfo?.country) params.set("country", opts.movieInfo.country);
  if (opts?.movieInfo?.language)
    params.set("language", opts.movieInfo.language);

  const url = `${API_BASE_URL}/llm/prompt/scene/${tmdbId}?${params}`;
