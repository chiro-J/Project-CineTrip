...
export const Landing = () => {
    ...
    const slidesData = [
        { id: 'A', text: '첫 번째 명장면 대사', movie: '영화 제목 1 | 촬영 장소', color: 'BDBDBD' },
        { id: 'B', text: '두 번째 명장면 대사', movie: '영화 제목 2 | 촬영 장소', color: '9E9E9E' },
        { id: 'C', text: '세 번째 명장면 대사', movie: '영화 제목 3 | 촬영 장소', color: '757575' },
        { id: 'D', text: '네 번째 명장면 대사', movie: '영화 제목 4 | 촬영 장소', color: '616161' },
    ];
    const totalSlides = slidesData.length;
    
    // 그리드용 목 데이터 추가
    const MOCK_GRID_IMAGES = [
        { id: 'grid-1', src: 'https://placehold.co/400x500/E0E0E0/333333?text=User+Trip+1', alt: '장소 이름 (영화 제목)', likes: 138 },
        { id: 'grid-2', src: 'https://placehold.co/400x500/E0E0E0/333333?text=User+Trip+2', alt: '장소 이름 (영화 제목)', likes: 102 },
        { id: 'grid-3', src: 'https://placehold.co/400x500/E0E0E0/333333?text=User+Trip+3', alt: '장소 이름 (영화 제목)', likes: 95 },
    ];
    ...
    useEffect(() => {
        slidesRef.current.forEach((slide, i) => {
            ...
            if (offset === 0) { // Center card
                ...
                scale = 1;
                opacity = 1;
            } else if (Math.abs(offset) === 1) { // Side cards
                translateX = offset * 300;
                translateZ = -200;
                scale = 0.8;
                opacity = 1;
            } else { // Hidden cards moving to the back
    ...
    useEffect(() => {
        ...
        if (viewport) {
            (viewport as HTMLDivElement).addEventListener('wheel', handleWheel, { passive: false });
        }
            ...
            <main className="relative">
                ...
                <section className="relative h-[70vh] flex flex-col items-center justify-center text-center px-6 overflow-hidden bg-gray-700 text-white">
                    <div className="relative z-10 space-y-4">
                        ...
                        <p className="max-w-sm mx-auto text-sm text-gray-300">
                            이곳 저곳 헤매며 찾지 말고, 영화 속 그곳을 CINE TRIP에서 바로 확인하세요.
                        </p>
                ...
                <section className="relative py-24 mt-12 text-black bg-white">
                    ...
                    <div className="mb-12 text-center">
                        ...
                        <p className="mt-2 text-sm text-gray-600">마우스 스크롤로 명장면을 넘겨보세요.</p>
                    </div>
                    <div id="carousel-viewport" ref={viewportRef} className="relative w-full h-[450px] mx-auto">
                            ...
                            {slidesData.map((slide, index) => (
                                ...
                                >
                                    <div className="relative text-center">
                                        <img src={`https://placehold.co/600x400/${slide.color}/333333?text=Main+Scene+${slide.id}`} alt={`메인 장면 ${slide.id}`} className="object-cover w-full max-w-md rounded-lg"/>
