...
const GalleryPage: React.FC<{ isOwner?: boolean }> = ({
  ...
  const isOwner = propIsOwner ?? (
    !userId ? true : // /profile/gallery 경로 - 로그인한 사용자가 소유자
  ...
  useEffect(() => {
    const fetchGalleryData = async () => {
      ...
      try {
        // 해당 유저의 게시물(사진) 가져오기
        const postsResponse = await fetch(`http://localhost:3000/api/posts?userId=${targetUserId}`);
        if (postsResponse.ok) {
        ...
        // 해당 유저의 북마크 가져오기 (영화)
        const bookmarksResponse = await fetch(`http://localhost:3000/api/user/${targetUserId}/bookmarks`);
  ...
  useEffect(() => {
    if (isOwner && user && userPhotosForGallery.length > 0) {
      ...
      const refreshGalleryData = async () => {
        try {
          const postsResponse = await fetch(`http://localhost:3000/api/posts?userId=${user.id}`);
          if (postsResponse.ok) {
            const posts = await postsResponse.json();
            const photoItems = posts.map((post: any) => ({
              id: post.id.toString(),
  ...
  const getPhotoData = (itemId: string) => {
    ...
    if (photoData) {
      ...
      return {
        ...
        image_url: photoData.image_url || photoData.src || "https://via.placeholder.com/400x300?text=No+Image",
